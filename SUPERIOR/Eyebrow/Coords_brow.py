...
Python file intended to obtain the coordinates of keypoints of the eyebrow movement to measure the tremor and the amplitude
of the forehead during a video. It uses YOLO for face detection and Mediapipe for a second face detection and
...
yolo_model = '/home/jhernandez2020/Escritorio/Facial_Mooveo/Mooveo1/yolov8n-face.pt'
...
def extract_coordinates(face_landmarks):
    frame_left_dict = {'brow': [], 'ref': []}
    frame_right_dict = {'brow': [], 'ref': []}
    for idx, landmark in enumerate(face_landmarks):
        if idx in leftEyebrowUpper:
            frame_left_dict['brow'].append((landmark.x, landmark.y, landmark.z))
            frame_left_dict['ref'].append((face_landmarks[168].x, face_landmarks[168].y, face_landmarks[168].z))
        elif idx in rightEyebrowUpper:
            frame_right_dict['brow'].append((landmark.x, landmark.y, landmark.z))
            frame_right_dict['ref'].append((face_landmarks[168].x, face_landmarks[168].y, face_landmarks[168].z))
    ...
    if ret:
        ...
        if face_roi and (None not in face_roi):
            ...
            for i in range(len(face_roi)):
                ...
                if detection_result.face_landmarks:
                    ...
                    for face_landmarks in detection_result.face_landmarks:
                        coords_left[f'Frame{fc}'] = \
                        extract_coordinates(face_landmarks)[0]
                        coords_right[f'Frame{fc}'] = \
                        extract_coordinates(face_landmarks)[1]
            ...
            if detection_result.face_landmarks:
                ...
                for face_landmarks in detection_result.face_landmarks:
                    coords_left[f'Frame{fc}'] = extract_coordinates(face_landmarks)[0]
                    coords_right[f'Frame{fc}'] = extract_coordinates(face_landmarks)[1]
